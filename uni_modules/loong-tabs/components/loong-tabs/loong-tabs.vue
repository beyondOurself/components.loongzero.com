<template>
	<view class="loong-swiper-page_wrap" :style="swiperBodyComputed">
		...
		<swiper class="swiper_body" @change="swiperChange" :current="swiperIndex">
			<swiper-item v-for="({ slot }, index) in swiperList" :key="index">
				<view class="swiper-block_box"><slot :name="`${slot || 'slot-' + (index + 1)}`"></slot></view>
			</swiper-item>
		</swiper>
	</view>
</template>
...
$loong-color-primary: $loong-primary !default;
$loong-sliding-line-height: 5rpx !default;
$loong-sliding-block-marging: 16rpx !default;
$loong-sliding-block-bottom-color: $loong-border-1 !default;
$loong-swiper-block-bgcolor: $loong-bg-color !default;
.loong-swiper-page_wrap {
	position: absolute;
	left: 0;
	right: 0;
	top: 0;
...
.swiper__sliding-blocks_box {
	white-space: nowrap;
	position: relative;
	padding: $loong-sliding-line-height 0;
...
.sliding-block_wrap {
	...
	margin: 0 $loong-sliding-block-marging;
	text-align: center;
	flex: 1;
}
.sliding-block_body {
...
.sliding-line_box {
	position: absolute;
	bottom: 0;
	height: $loong-sliding-line-height;
	border-radius: 10rpx;
	transition: left 0.3s ease;
	background-color: $loong-color-primary;
}
.sliding--active {
	font-weight: bold;
	color: $loong-color-primary;
}
.swiper-block_box {
	height: 100%;
	background-color: $loong-swiper-block-bgcolor;
	border-top: solid 1px $loong-sliding-block-bottom-color;
}
