 ...
 * @LastEditTime: 2025-01-24 11:39:25
	...
	<view class="loong-picker">
		...
		<view class="picker_input">
			<slot :data="{ modelValue }">
				<input disabled class="picker_input_body" :class="{ 'is--form-item': isFormItem }" :value="inputValueGet" :placeholder="placeholderGet" @click.stop="open" />
			</slot>
		</view>
		<!-- E 输入框 -->

		<!-- <view class="picker_mask" v-show="isOpened"  @click.stop="triggerMask"></view> -->
		<view class="picker_mask" v-show="isOpened"  :style="pickerMaskStyle"  @click.stop="triggerMask"></view>

		<!-- S 遮罩 -->

		<view class="picker_wrap"  v-show="isOpened" :style="pickerMaskStyle">
			<!-- E 遮罩 -->
					...
					<template v-for="(option, optionIndex) in curOptionsGet" :key="optionIndex">
						<picker-view-column>
							<view
								...
								:key="itemIndex"
							>
								<slot name="item" :data="item" :index="itemIndex">{{ item.label }} {{activatedIndexList}}</slot>
							</view>
						</picker-view-column>
					</template>
				</picker-view>
